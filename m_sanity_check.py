...
def question_1h_sanity_check(highway):
    ...
    # highway.forward()
    # inpt = torch.tensor([[[1,2,1,2], [1,2,3,4], [8,7,6,5], [3,2,4,1]]])
    # TODO add more test case: all 0, all negative, etc.
    inpt = torch.tensor([[0,-1,2,3], [4,-2,3,1], [3,-4,-7,9]], dtype=torch.float)
    highway.x_projection.weight = Parameter(torch.tensor([[1,-2,0,2], [1,-2,3,4], [8,7,-6,5], [3,2,4,1]], dtype=torch.float))
    highway.x_projection.bias = Parameter(torch.tensor([1,-2,0,2], dtype=torch.float))
    highway.x_gate.weight = Parameter(torch.tensor([[0.1, -0.3, 2, 1], [1.2,-0.2,2.3,1.4], [8.2,-0.7,3.6,0.05], [2.3,-7.2,1.4,0.01]], dtype=torch.float))
    highway.x_gate.bias = Parameter(torch.tensor([1.2,-3.2,-0.2,6.2], dtype=torch.float))
    x_highway = highway.forward(inpt)

    x_highway_ref = torch.tensor([[ 8.9982e+00,  1.7943e+01,  7.7903e-04,  1.1000e+01],
        [ 1.0999e+01,  1.8999e+01,  5.0000e+00,  2.3000e+01],
        [ 5.4603e+00, -1.4486e+00,  7.5851e+01,  0.0000e+00]],
       dtype=torch.float)

    assert(torch.sum(x_highway_ref-x_highway) <0.5), "WRONG VALUE"
    print("x_highway = ", x_highway)
...
def main():
    ...
    highway = Highway(E_WORD_SIZE)
    # m_word_size, e_char_size, k_kernel_size, f_filters
    # cnn = CNN()
